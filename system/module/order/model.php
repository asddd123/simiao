...
class orderModel extends model
{
    ...
    public function create1()
    {
        ...
        $count = $this->dao->insert(TABLE_ORDER)
            ->data($order, 'createAddress,deliveryAddress,zipcode,phone,contact,price,count,product,name,userName')
            ->autocheck()
            ->batchCheck($this->config->order->require->create, 'notempty')
            ->exec();
        if(dao::isError()) return array('result' => 'fail', 'message' => dao::getError());
...
//            $goods->count       = $this->post->count[$product->id];
            ...
            $this->dao->insert(TABLE_ORDER_PRODUCT)->data($goods)->autoCheck()->exec();
            $this->dao->update(TABLE_PRODUCT)->set('status')->eq('offline')
                                             ->set('startDate')->eq(time())
                                             ->set('endDate')->eq(time()+365*24*60*60)
                                             ->set('userName')->eq($post->userName)
                                             ->where('id')->eq($product->id)->exec();
        }
