...
class order extends control
{
    ...
    public function confirm1($productID)
    {
        $this->LoadModel('product');
    ...
    public function edit($orderID)
    {
        ...
        if($_POST)
        {
            ...
            $this->send(array('result' => 'success', 'message' => $this->lang->saveSuccess , 'locate' =>helper::createLink('product','admin')));
        }
        $this->view->expressList = $this->loadModel('tree')->getOptionMenu('express');
        $this->view->order       = $order;
        $this->view->products    = $this->order->getOrderProducts($orderID);
    ...
    public function test() {
        $return = $this->order->create1();
        $this->send(array('result' => 'success', 'message' => 'success', 'locate' => inlink('admin')));
    }
}
